require 'atmos/plugin_manager'
require 'atmos/plugins/output_filter'

describe Atmos::PluginManager do
  ...
  describe "load_plugin" do
    ...
    it "allows plugin loading to fail" do
      ...
      expect(Atmos::Logging.contents).to match(/Failed to load atmos plugin/)
    end
  ...
  describe "output_filters" do
    ...
    it "instantiates output filters" do
      f1 = Class.new(Atmos::Plugins::OutputFilter)
      f2 = Class.new(Atmos::Plugins::OutputFilter)
      pm = described_class.new([])
      ...
      expect(filters).to be_a_kind_of(Atmos::PluginManager::OutputFilterCollection)
    end

  end

  describe Atmos::PluginManager::OutputFilterCollection do

    describe "filter_block" do

      it "returns a block which applies each filter in turn" do
        f1 = Atmos::Plugins::OutputFilter.new({})
        f2 = Atmos::Plugins::OutputFilter.new({})
      ...
      it "returns a block which ignores failures in filters" do
        ...
        expect(Atmos::Logging.contents).to match(/Output filter failed during filter/)
      end
    ...
    describe "close" do
      ...
      it "allows failure when calling close on each filter" do
        ...
        ofc = described_class.new([f1, f2])
        ofc.close
        expect(Atmos::Logging.contents).to match(/Output filter failed during close/)
      end
